...
namespace sibfalcerueu
{
    public partial class BarangMasuk : MetroForm
    {
        ...
        {
            metroPanel2.Location = new Point(155, 252);
            metroPanel1.Location = new Point(155, 211);
            if (pil == "panel1")
            {
        ...
        private void kode()
        {
            ...
            if (this.OpenConnection() == true)
            {
                ...
                cmd = new MySqlCommand("select kode from barang_masuk where kode in(select max(kode) from barang_masuk) order by kode desc", conn);
        ...
        private void btninput_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrWhiteSpace(txtfaktur.Text))
            {
        ...
        private void metroGrid2_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            txtkodesupplier.Text = row.Cells[0].Value.ToString();
            metroPanel1.Visible = false;
        }
        ...
        private void txtkodesupplier_Click(object sender, EventArgs e)
        {
            visiblepanel("panel1");
        }
        ...
        private void metroPanel2_VisibleChanged(object sender, EventArgs e)
        {
            ...
            {
                txtkodesupplier.Visible = false;
            }
            else
            {
                txtkodesupplier.Visible = true;
            }
        }
        ...
        {
            string query = "INSERT INTO barang_masuk VALUES (@kode, @nofaktur, @tanggal, @kodeuser, @kodesupplier)";
            if (this.OpenConnection() == true)
            {
                //Start MysqlCommand
                cmd = new MySqlCommand(query, conn);
                ...
                cmd.Parameters.AddWithValue("@nofaktur", txtfaktur.Text);
                cmd.Parameters.AddWithValue("@tanggal", DateTime.Today);
                cmd.Parameters.AddWithValue("@kodeuser", txtkodeuser.Text);
                cmd.Parameters.AddWithValue("@kodesupplier", txtkodesupplier.Text);
            ...
            if (this.OpenConnection() == true)
            {
                foreach (DataGridViewRow row in metroGrid1.Rows)
                {
                    try
                    {
                        ...
                        cmd.CommandText = "INSERT INTO detail_barang_masuk VALUES (@kode, @jumlah, @kode_barang)";
                        cmd.ExecuteNonQuery();
                    }
                    catch (Exception ex)
                    {
        ...
        private void btnsubmit_Click(object sender, EventArgs e)
        {
            if(this.Insert1() == true)
            {
                if(this.Insert2() == true)
                {
                    ...
                    txtkodebarang.Text = "";
                    txtjumlah.Text = "";
                    MetroMessageBox.Show(this, "Submit Transaksi Barang Masuk Berhasil", "Transaksi Sukses", MessageBoxButtons.OK, MessageBoxIcon.Question);
                }else
                {
